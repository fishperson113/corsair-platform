...
- Pipeline is a three-agent sequence on the **OpenAI Agents SDK** (`@openai/agents`, `agents.ts`): Analyst → Matcher → Writer (model `gpt-4o-mini`, zod-typed outputs). Requires `OPENAI_API_KEY` (a placeholder is set on cloud — replace with the real key). Candidate profile in `pipeline.ts` is a placeholder to replace with real evidence. The intelligence is OpenAI-only per the owner's choice — do not add Claude here.
