...
- Pipeline is a three-agent sequence on the **OpenAI Agents SDK** (`@openai/agents`, `agents.ts`): Analyst → Matcher → CV Tailor (model `gpt-4o-mini`). The evidence is the owner's real LaTeX CV in the `CV_TEX` secret; the tailor edits title/objective/skills/sections to fit the job without fabricating, then the LaTeX is compiled to PDF by the owner's self-hosted Tectonic service (`services/latex-compiler`, secret `LATEX_COMPILE_URL`) and the PDF is uploaded to Drive. Placeholders are set on cloud for `OPENAI_API_KEY`, `CV_TEX`, `LATEX_COMPILE_URL` — replace with real values. Intelligence is OpenAI-only per the owner's choice — do not add Claude here.
