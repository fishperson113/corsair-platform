...
- Pipeline is a three-agent sequence on the **OpenAI Agents SDK** (`@openai/agents`, `agents.ts`): Analyst → Matcher → CV Tailor (model `gpt-4o-mini`). The owner's CV is a **multi-file LaTeX project** that lives in the self-hosted Tectonic service (`services/latex-compiler`, holds `cv/` = main.tex + preamble + config/ + sections/). The client fetches tailorable sections via `GET /source`, the tailor edits header/summary/skills/experience/projects without fabricating, and the client posts them to `POST /compile` for a PDF uploaded to Drive. Secret `LATEX_COMPILE_URL` points at the service; `OPENAI_API_KEY` has a placeholder on cloud — replace both with real values. Intelligence is OpenAI-only per the owner's choice — do not add Claude here.
